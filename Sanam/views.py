...
def eventsForCat(request, category_id):
    return render_to_response("eventlistforCat.html",
...
def eventsForSubCat(request, subcategory_id):
    return render_to_response("eventlistforCat.html", {'events': Event.objects.filter(subcategory__pk=subcategory_id),
...
def editCategory(request, subcategory_id):
    if request.method == 'GET':
        return render_to_response("CategoryAdd.html",
                                  {'form': CategoryModelForm(instance=Category.objects.filter(pk=subcategory_id)[0]),
                                   'subcategories': Subcategory.objects.all(), 'category': Category.objects.all()},
                                  context_instance=RequestContext(request))
    else:
        form = SubCategoryModelForm(request.POST, request)
        form.save()
        return render_to_response("CategoryAdd.html", {'success': True, 'form': CategoryModelForm(
            instance=Category.objects.filter(pk=subcategory_id)[0]), 'subcategories': Subcategory.objects.all(),
                                                       'category': Category.objects.all()},
                                  context_instance=RequestContext(request))
...
def Pardakht(request, ev_id):
    ...
    for field in event.ticktype.all():
        if int(request.POST[field.title]) > field.capacity:
            return render_to_response("error.html", {'remain': field.capacity, 'type': field.title},
                                      context_instance=RequestContext(request));
    for field in event.ticktype.all():
    ...
    for field in event.ticktype.all():
        ...
        total = total + x
    return render_to_response("Bank.html", {'total': total, 'ev_id': ev_id, 'form': request.POST},
                              context_instance=RequestContext(request))
...
def TrackingCode(request):
    ...
    for field in tempTicket.objects.all():
        ...
        y.orderList = x
        y.save()
    return render_to_response("TrackingCode.html", {'orderList': x, 'tickets': tempTicket.objects.all()},
                              context_instance=RequestContext(request))
...
def printpage(request, order):
    ...
    return render_to_response("print.html", {'tickets': ticks, 'orderNum': order},
                              context_instance=RequestContext(request))
...
def AdminProfMinsearch(request):
    ...
    return render_to_response("c_adminprofile.html",
                              {'events': events, 'sell': sell, 'unsell': nosel, 'cost': allcost},
                              context_instance=RequestContext(request))
