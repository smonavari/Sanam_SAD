...
class EventModelForm(forms.ModelForm):
    class Meta:
        model = Event
        fields = ['title', 'address', 'subcategory', 'endDate', 'startTime',
                  'seller', 'dscp']
        labels = {
            'title': 'عنوان',
            'address': 'آدرس',
...
TicketFormSet = inlineformset_factory(Event, TicketType, fields=('location', 'price', 'capacity', 'time'), extra=1,
                                      widgets={'location': forms.TextInput(attrs={'placeholder': 'مکان'}),
                                               ...
                                               'time': forms.TextInput(attrs={'type': 'DateField'}),
